...
;; copied from bitvector.rkt and hacked up...
...
(struct float (eb sb)
  ...
  [(define (least-common-supertype self other) (if (equal? self other) self @any/c))
   (define (type-name self) (string->symbol (format "float~a,~a?" (float-eb self) (float-sb self))))
   ...
   (define (type-eq? self u v)        (@fpeq u v))
   (define (type-equal? self u v)     (@fpeq u v))
...
(struct fp (sign exponent significand type)
  #:transparent
  #:methods gen:typed
  [(define (get-type self) (fp-type self))]
  #:methods gen:custom-write
  [(define (write-proc self port mode)
     (fprintf port "(fp ~a ~a ~a ~a)" 
              (fp-sign self)
              (fp-exponent self)
              (fp-significand self)
              (fp-type self)))])
...
(define-match-expander @fp
  (syntax-rules ()
    [(_ sign-pat exponent-pat significand-pat) (fp sign-pat exponent-pat significand-pat)])
  (make-variable-like-transformer #'make-fp))
